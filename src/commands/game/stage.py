...
def run(stage_id, difficulty, kagi=None):
    ...
    if not stage_id.isnumeric():
        try:
            stage_id = str(config.Quests.where('name', 'like', '%' + stage_id + '%').first().id)
        except:
    ...
    if friend['is_cpu'] == False:
        ...
        if kagi != None:
            sign = json.dumps(
                {'difficulty': int(difficulty), 'eventkagi_item_id': kagi, 'cpu_friend_id': int(friend['id']),
                 'is_playing_script': True, 'selected_team_num': int(config.deck)})
        else:
            sign = json.dumps(
                {'difficulty': int(difficulty), 'cpu_friend_id': int(friend['id']), 'is_playing_script': True,
                 'selected_team_num': int(config.deck)})
