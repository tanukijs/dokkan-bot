...
import config
from commands.get_remaining_stones import get_remaining_stones_command
    ...
    while True:
        ...
        if event == 'SUMMON' and len(values['GASHAS']) > 0:
            ...
            if values[0]:
                ...
                url = config.gb_url + '/gashas/' + str(summon_id) + '/courses/2/draw'
                window.Hide()
                window.Refresh()
                ...
                url = config.gb_url + '/gashas/' + str(summon_id) + '/courses/1/draw'
                window.Hide()
                window.Refresh()
                for i in range(int(values['LOOP'])):
                    r = requests.post(url, headers=headers).json()
